...
search_directory_DB(){
	...
	while : ; do
		TOTAL=`/usr/syno/pgsql/bin/psql mediaserver -U postgres -tA -c "select count(1) from directory where lower(path) like '%$PATH_MEDIA_SQL%'"`
...
search_file_DB(){
    ...
    TOTAL=`/usr/syno/pgsql/bin/psql mediaserver -U postgres -tA -c "select count(1) from video where lower(path) like '%$FICH_MEDIA_SQL%'"`
